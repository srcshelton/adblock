        ...
        this._check_box.
            change(function () {
                ...
                if (checked) {
                  ...
                  if (FilterListUtil.cached_subscriptions[id].subscribed) {
...
FilterListUtil.updateCheckbox = function (filter_list, id) {
    ...
    // If neccessary, check / uncheck the AA option on the 'General' tab.
    if (id === 'acceptable_ads' && filter_list.subscribed && $('#acceptable_ads').is(':checked') !== filter_list.subscribed) {
...
function addMalwareNotificationDiv() {
  ...
  chrome.storage.local.get('malware-notification', function (response) {
...
var filtersInit = function () {
    ...
    chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
        ...
        // If the number of subscription differ, just reload the page.
        if (Object.keys(subs).length !== Object.keys(cached_subscriptions).length) {
            ...
            if (entry.subscribed) {
              if (entry.last_update && entry.last_update_failed_at) {
                ...
                if (parseInt(entry.last_update) > parseInt(entry.last_update_failed_at)) {
                  if (subs[id].last_update_failed_at) {
                    delete subs[id].last_update_failed_at;
                  }
                } else {
                  if (subs[id].last_update) {
