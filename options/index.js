...
function setUserId() {
  var paymentHREFhref = 'https://getadblock.com/pay/?source=O&u=' + BG.STATS.userId();
  $('#paymentlink').attr('href', paymentHREFhref);
}

function displayTranslationCredit() {

  var fullLang = chrome.i18n.getUILanguage().toLowerCase();
  if (fullLang.substring(0, 2) != 'en') {
    var translators = [];
...
function displayVersionNumber() {
  $('#version_number').text(translate('optionsversion', [BG.chrome.runtime.getManifest().version]));
}
