...
if (!frameData || Object.keys(frameData['0'].resources).length === 0) {
  ...
  BG.get_custom_filters_text(function (filters) {
      ...
      var processedData = BG.process_frameData(frameData);
      ...
      BG.add_to_matchCache(matchCache);
      addRequestsToTables(processedData);
    });  // end of get_custom_filters_text
}
...
function sortTable() {
  ...
  $('td:nth-of-type(' + columnNumber + ')', table).each(function (index, element) {
      cellList.push(element.textContent.toLowerCase() + 'ÿÿÿÿÿ' + (index + 10000));
