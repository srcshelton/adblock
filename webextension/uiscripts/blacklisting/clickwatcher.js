...
ClickWatcher.prototype.show = function () {
  var _this = this;
  var wait = $('<div></div>').
    text(translateAndRemoveHTML('findingads')).
    dialog({
  ...
  window.setTimeout(function () {
    _this._ui = _this._build_ui();
    wait.dialog('close');
    wait.remove();
    _this._ui.dialog('open');
    _this._highlighter.enable();
  }, 10);
};
...
ClickWatcher.prototype._onClose = function () {
  if (this._clickedElement == null) {
    ...
    this._fire('click', this._clickedElement);
  }
...
ClickWatcher.prototype._build_ui = function () {
  ...
  function clickCatch(element) {
    _this._clickedElement = _this._highlighter.getCurrentNode(element);

    //_this._clickedElement = element;
    _this._ui.dialog('close');
    return false;
  }
