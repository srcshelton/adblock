...
var shown = {};
var socialMediaSharePanelState = "initial";
...
function customize_for_this_tab(info) {
    ...
    if (info.adblock_is_paused) {
      ...
      BGcall("get_settings", function(info) {
        if (info.show_block_counts_help_link) {
...
//      if (eligible_for_undo && BGcall("count_cache.getCustomFilterCount(url_to_check_for_undo))
//          show(["div_undo", "separator0"]);
        ...
        if (info.youtube_channel_whitelist && host === "www.youtube.com"
            && /channel|user/.test(url) && eligible_for_undo) {
...
$(function() {
    ...
    $("#div_report_an_ad").click(function() {
        BGcall("getCurrentTabInfo", function(info) {
            BGcall("openAdReportTab", escape(info.tab.url));
            addon.port.emit("close");
        });
    });
...
$( document ).ready(function() {
    addon.port.on("update_content",function(info) {
        ...
        localizePage();
        var popupHeight = parseInt(document.body.offsetHeight) + 10;
        addon.port.emit("resizePopup", {width: ADBLOCK_BUTTON_PANEL_WIDTH, height:popupHeight});
    });
