...
BlockingFilterSet = function (patternFilterSet, whitelistFilterSet, domainArray) {
...
BlockingFilterSet.prototype = {
  ...
  matches: function (url, elementType, frameDomain, returnFilter, returnTuple) {
    ...
    if (this.domains &&
        urlDomain &&
        this.domains[urlDomain.charAt(0)] &&
        this.domains[urlDomain.charAt(0)].indexOf(urlDomain) > -1) {
      if (loggingEnable) {
        log('matched domain', urlDomain);
      }
