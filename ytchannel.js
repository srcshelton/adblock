...
// 1) Reload the stylesheet when we recieve a message from the background page to do so.
// 2) Query the YouTube page for channel names, and send them to the background page to white-list
function onMessage(msg)
{
  ...
  if (msg.type === "whitelistAllYouTubeChannels")
  {
    ...
    for (var inx = 0; inx < myElements.length; inx++)
    {
      ...
      var theTitle = theElement.innerHTML
      if (theTitle)
      {
        titles.push(parseChannelName(theTitle.trim()));
      }
