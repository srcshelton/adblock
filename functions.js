...
// Compatibility
var storageSet = storage_set;

var isEqual = function (x, y) {
  if ((typeof x == 'object' && x != null) && (typeof y == 'object' && y != null)) {
    if (Object.keys(x).length != Object.keys(y).length) {
      console.log('object keys length differ', Object.keys(x), Object.keys(y));
      return false;
    }
    ...
    for (var prop in x) {
      if (y.hasOwnProperty(prop)) {
        if (!isEqual(x[prop], y[prop])) {
          console.log('property values differ', prop, x[prop], y[prop]);
          return false;
        }
      } else {
        console.log("y doesn't have property ", prop);
        return false;
      }
    }
