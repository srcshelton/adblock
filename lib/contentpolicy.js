...
com.getadblock.policy = Class({
    ...
    shouldLoad: function (aContentType,
        ...
        if (!aContext || !functions.isValidUri(aContentLocation))
            return Ci.nsIContentPolicy.ACCEPT;

        // Popup blocking
        if (aContentType === Ci.nsIContentPolicy.TYPE_DOCUMENT) {
           var window = aContext.contentWindow.top;
           if (window.opener) {
               var opener = aRequestOrigin.spec;
               var url = aContentLocation.spec;
               if (!opener || !url) {
                   return Ci.nsIContentPolicy.ACCEPT;
               }
               var blocked = _myFilters.blocking.matches(url, MY.ElementTypes.popup, functions.parseUri(opener).host);
               if (blocked) {
                   window.close();
        ...
        if (aContext instanceof components.interfaces.nsIDOMNode) {
            ...
            if (contentWin) {
                ...
                if (selectedTab) {
                    ...
                    var matchGeneric = _myFilters.blocking.whitelist.matches(aContentLocation.spec, MY.ElementTypes.genericblock, aRequestOrigin.host);
                    var blocked = _myFilters.blocking.matches(aContentLocation.spec, elType, aRequestOrigin.host, matchGeneric);
