...
var get_content_script_data = function (options, sender) {
    ...
    if (hiding) {
        ...
        var matchGeneric = CP._myFilters.blocking.whitelist.matches(sender.tab.url, CP.MY.ElementTypes.generichide, sender.tab.url);
        result.selectors = CP._myFilters.hiding.filtersFor(options.domain, matchGeneric);
    }
    return result;
};
...
exports.get_frameData = get_frameData;
...
// This function should only be used by the adreport page.
...
var getDebugInfo = function () {
    ...
    for (var setting in settings)
        the_debug_info.settings.push(setting + ": " + get_settings()[setting]);

    // We need to hardcode malware-notification setting,
    // because it isn't included in _settings object, but just in localStorage
    the_debug_info.settings.push("malware-notification: " + functions.storage_get('malware-notification'));
    the_debug_info.settings = the_debug_info.settings.join('\n');

    var customFilters = functions.storage_get("custom_filters") || "no custom filters";
    the_debug_info.custom_filters = "==== Custom Filters ====\n" + customFilters;
    ...
    the_debug_info.other_info = info.join('\n');
