...
var getTypeName = function(type) {
  switch(type) {
    ...
    case MY.ElementTypes.script: return "script";
    case MY.ElementTypes.background:
    case MY.ElementTypes.image: return "image";
    case MY.ElementTypes.stylesheet: return "stylesheet";
    case MY.ElementTypes.object: return "object";
    case MY.ElementTypes.subdocument: return "subdocument";
    case MY.ElementTypes.object_subrequest: return "object_subrequest";
    case MY.ElementTypes.media: return "media";
    case MY.ElementTypes.xmlhttprequest: return "xmlhttprequest";
    case MY.ElementTypes.other: return "other";
    //Cheating with $document & $elemhide here to make it easier: they are considered 'the same'
    case MY.ElementTypes.document | MY.ElementTypes.elemhide: return "page";
    case MY.ElementTypes.popup: return "popup";
...
var createResourceblockFilterset = function(id, text) {
    ...
    for (var i=0; i<text.length; i++) {
        var filter = MY.Filter.fromText(text[i], true);
        if (MY.Filter.isSelectorFilter(text[i]))
            h[filter.id] = filter;
        else if (MY.Filter.isWhitelistFilter(text[i]))
            w[filter.id] = filter;
        else if (text[i])
            b[filter.id] = filter;
    }
    local_filtersets[id].hiding = MY.FilterSet.fromFilters(h);
    local_filtersets[id].blocking =
        new MY.BlockingFilterSet(MY.FilterSet.fromFilters(b), MY.FilterSet.fromFilters(w));
}
...
var getResourceBlockData = function(opts) {
    ...
    createResourceblockFilterset("build_in_filters", MY.MyFilters.prototype.getExtensionFilters(data.settings));
    ...
    if (opts.tabId) {
        ...
        for (var thisFrame in loaded_frames) {
            ...
            if (Number(thisFrame) === 0) {
                ...
                resources[frame.url] = {
                    type: MY.ElementTypes.document | MY.ElementTypes.elemhide,
                    domain: frame.domain,
                    resource: frame.url
                };
            }
            ...
            for (var res in frame.resources) {
                if (/^HIDE\:\|\:.+/.test(res)) {
                    ...
                    if (elemType === MY.ElementTypes.document)
                        continue;
      ...
      resources[opts.resource] = {
        type: MY.ElementTypes[opts.itemType],
    ...
    for (var i=0; i<filters.length; i++) {
        try {
            var normalized = MY.FilterNormalizer.normalizeLine(filters[i]);
    ...
    for (var i in resources) {
        ...
        // selector (e.g., domain.com##.ad). Neither do we know the filter list
            ...
            for (var fset in local_filtersets) {
                ...
                if (currentlist_matchingfilter) {
                    ...
                    if (MY.Filter.isWhitelistFilter(currentlist_matchingfilter))
                        break;
                }
            }
        }
        ...
        if (matchingfilter) {
            if (MY.Filter.isWhitelistFilter(matchingfilter))
                type = {sort:2, name:'whitelisted'};
            else if (MY.Filter.isSelectorFilter(i)) {
        ...
        var isThirdParty = (type.name === 'hiding' ? false :
            MY.BlockingFilterSet.checkThirdParty(resources[i].domain, resourceDomain));
        var cellFive = {};
...
var filterMatchesResource = function(filter, url, type, domain) {
  var temp_filterset = new MY.BlockingFilterSet(
                        MY.FilterSet.fromFilters({1: MY.Filter.fromText(filter) }),
                        MY.FilterSet.fromFilters({}));
