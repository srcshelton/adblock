...
MyFilters.prototype._load_default_subscriptions = function () {
    ...
    function listIdForThisLocale() {
        ...
        switch (language) {
            case 'ar': return 'easylist_plus_arabic_plus_french';
...
MyFilters.prototype._make_subscription_options = function () {
    ...
    return {
        ...
        "easylist_plus_arabic_plus_french": { //  EasyList + Arabic & French filters
...
var FilterNormalizer = {
    ...
    normalizeLine: function (filter) {
        ...
        if (Filter.isSelectorFilter(filter)) {
            ...
            try {
                ...
                throw new Error("Invalid CSS selector syntax: " + filter);
            }
            ...
            if (!Filter.isWhitelistFilter(filter) && hasWhitelistOptions)
                throw new Error("$document and $elemhide may only be used on whitelist filters: " + filter);
    ...
    _old_style_hiding_to_new: function (filter) {
        ...
        if (rule.length == 0 || !/^(?:\*|[a-z0-9\-_]*)(?:\([^=][^\)]*?\))*$/i.test(rule))
            throw new Error("bad selector filter: " + filter);
    ...
    _checkForObjectProperty: function (text) {
        if (text in Object)
            throw new Error("Filter causes problems in the code: " + text);
    },
...
ElementTypes.convertFireFoxContentType = function (contentType, node) {
    switch (contentType) {
        ...
        case ElementTypes.FireFoxElementTypes.TYPE_WEBSOCKET:
            return ElementTypes.websocket;
        default:
            return ElementTypes.NONE;
    }
};
...
FilterSet.prototype = {
    ...
    _viewFor: function (domain, matchGeneric) {
        var result = new FilterSet();
        if (typeof matchGeneric === 'undefined' ||
            matchGeneric === null) {
    ...
    matches: function (url, elementType, frameDomain, isThirdParty, matchGeneric) {
        ...
        for (var k in limited.items) {
            ...
            for (var i = 0; i < entry.length; i++) {
                ...
                if (filter === undefined || !filter.matches(url, elementType, isThirdParty))
                    continue; // no match
