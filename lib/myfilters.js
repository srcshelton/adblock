...
MyFilters.prototype.fetch_and_update = function(id, isNewList) {
    ...
    xhr.setRequestHeader("X-Client-ID", "AdBlock/" + require("stats").STATS.version);
    xhr.setRequestHeader("If-Modified-Since", this._subscriptions[id].last_modified || undefined);
    xhr.onload = function() {
        ...
        if (xhr.status == 304) {
            functions.log("List not modified " + url);
            ...
            functions.log("Filter List fetched, but invalid list " + url);
            onError();
        }
    }
    xhr.onerror = function(err) {
        functions.log("Error condition, url",url,"status", xhr.status, "statusText", xhr.statusText,"error",JSON.stringify(err));
        //TODO - notify user?
    }
    xhr.send();
}
...
*/
if (typeof exports !== 'undefined') exports.MyFilters = MyFilters;
...
if (typeof exports !== 'undefined') exports.DomainSet = DomainSet;
var FilterNormalizer = {
  ...
  normalizeList: function(text, keepComments) {
    ...
    for (var i=0; i<lines.length; i++) {
      try {
        ...
        functions.log("Filter '" + lines[i] + "' could not be parsed: " + ex);
        ignoredFilterCount++;
      }
    }
    if (ignoredFilterCount)
      functions.log('Ignoring ' + ignoredFilterCount + ' rule(s)');
    return result.join('\n') + '\n';
  },
  ...
  normalizeLine: function(filter) {
    ...
    if (/#[\*a-z0-9_\-]*(\(|$)/.test(filter) && !/\#\@?\#./.test(filter)) {
      ...
      functions.log('Converted ' + oldFilter + ' to ' + filter);
    }
...
if (typeof exports !== 'undefined') exports.FilterNormalizer =  FilterNormalizer;
...
ElementTypes.convertFireFoxContentType = function(contentType, node) {
    switch (contentType) {
        case ElementTypes.FireFoxElementTypes.TYPE_OTHER: return ElementTypes.other;
        case ElementTypes.FireFoxElementTypes.TYPE_SCRIPT: return ElementTypes.script;
        case ElementTypes.FireFoxElementTypes.TYPE_IMAGE: return ElementTypes.image;
        case ElementTypes.FireFoxElementTypes.TYPE_CSSIMAGE: return ElementTypes.image;
        case ElementTypes.FireFoxElementTypes.TYPE_FAVICON: return ElementTypes.image;
        case ElementTypes.FireFoxElementTypes.TYPE_STYLESHEET: return ElementTypes.stylesheet;
        case ElementTypes.FireFoxElementTypes.TYPE_OBJECT: return ElementTypes.object;
        case ElementTypes.FireFoxElementTypes.TYPE_DOCUMENT: return ElementTypes.document;
        case ElementTypes.FireFoxElementTypes.TYPE_SUBDOCUMENT: return ElementTypes.subdocument;
        case ElementTypes.FireFoxElementTypes.TYPE_REDIRECT: return ElementTypes.NONE;
        case ElementTypes.FireFoxElementTypes.TYPE_REFRESH: return ElementTypes.NONE;
        case ElementTypes.FireFoxElementTypes.TYPE_XBL: return ElementTypes.NONE;
        case ElementTypes.FireFoxElementTypes.TYPE_PING: return ElementTypes.NONE;
        case ElementTypes.FireFoxElementTypes.TYPE_XMLHTTPREQUEST: return ElementTypes.xmlhttprequest;
        case ElementTypes.FireFoxElementTypes.TYPE_OBJECT_SUBREQUEST: return ElementTypes.object_subrequest;
        default:
...
if (typeof exports !== 'undefined') exports.ElementTypes = ElementTypes;
if (typeof exports !== 'undefined') exports.FilterOptions = FilterOptions;
function FilterSet() {
...
BlockingFilterSet.prototype = {
  ...
  matches: function(url, elementType, frameDomain, returnFilter) {
    ...
    if (match) {
      //functions.log(frameDomain, ": whitelist rule", match._rule, "exempts url", url);
      this._matchCache[key] = (returnFilter ? match._text : false);
      return this._matchCache[key];
    }
    match = this.pattern.matches(url, elementType, frameDomain, isThirdParty);
    if (match) {
      //functions.log(frameDomain, ": matched", match._rule, "to url", url);
...
if (typeof exports !== 'undefined') exports.FilterSet = FilterSet;
if (typeof exports !== 'undefined') exports.BlockingFilterSet = BlockingFilterSet;
var Filter = function() {
...
if (typeof exports !== 'undefined') exports.PatternFilter = PatternFilter;
if (typeof exports !== 'undefined') exports.SelectorFilter = SelectorFilter;
if (typeof exports !== 'undefined') exports.Filter = Filter;