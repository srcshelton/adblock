...
var openOptionsTab = function () {
    ...
    if (args && args.length >= 2) {
      localOptionsPageURL = localOptionsPageURL + "?" + args[0];
    }
...
//OPTIONS PAGES SECTION
var openBugReportPage = function () {
    var localBugReportPageURL = "options/bug-report.html";
    var bugReportTabWorker = null;
    var onReadyBugReportListener = function (tab) {
        if (data.url(localBugReportPageURL) === tab.url) {
            bugReportTabWorker = tab.attach({
                contentScriptFile: [
                    data.url("jquery/jquery-2.1.1.min.js"),
                    data.url("jquery/jquery-ui.min.js"),
                    data.url("html_parser.js"),
                    data.url("cs_functions.js"),
                    data.url("cs_port.js"),
                    data.url("options/bug-report.js")
                ],
            });
            port_ff.chrome.startListening(bugReportTabWorker);
        } else {
            tab.removeListener("ready", onReadyBugReportListener);
        }
    };
    var bugReportTab = tabs.open({
        url: data.url(localBugReportPageURL),
        onReady: onReadyBugReportListener
    });
};
exports.openBugReportPage = openBugReportPage;
...
buttonPanel.port.on("popupOpened", function () {
    require("./servermessages").recordGeneralMessage("slposh opened");
});
