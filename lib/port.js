...
} // end if (typeof OPERA == "undefined")
if (typeof exports !== 'undefined') exports.OPERA = OPERA;

if (typeof CHROME === "undefined") {
    (function(globals) {
        globals.CHROME = false;
    })(this);
    if (typeof exports !== 'undefined') exports.CHROME = CHROME;
...
if (typeof FIREFOX === "undefined") {
    ...
    (function(globals) {
        ...
        var handleContentScriptError = function(worker, error) {
            ...
            str += stack;
            //STATS.msg(str);
            require("functions").log(str);
        };

        var detachWorker = function(worker, workerArray) {
            var index = workerArray.indexOf(worker);
            if (tabWorkers[index] && tabWorkers[index].id) {
                var tab = require('sdk/tabs/utils').getTabForId(tabWorkers[index].id);
                if (tab) {
            ...
            if (tabWorkers[index])
                remove(tabWorkers, tabWorkers[index]);
        };
            ...
            //a seperate array of tabs, that must match the above workers array.  We keep a separate array
            //to allow us to zero out the adblock specific items we store on each tab
            //for example: (_getadblock_com_blockcount)
            //the tab object on the worker object doesn't have the correct reference when we need to zero it out.
            var tabWorkers = [];
            ...
            globals.chrome = {
                ...
                startListening: function(worker) {
                    ...
                    add(workers, worker);
                    add(tabWorkers, worker.tab);
                    worker.on('pageshow', function() {
                        add(workers, this);
                        add(tabWorkers, this.tab);
                    });
                    ...
                    worker.port.on("call", function(request) {
                        ...
                        if (typeof fn === 'undefined') {
                            ...
                            fn = MY.FilterNormalizer[request.fn];
                            if (typeof fn === 'undefined') {
                                fn = MY.Filter[request.fn];
                            }
                            if (typeof fn === 'undefined') {
                                fn = MY.PatternFilter[request.fn];
                            }
                            if (typeof fn === 'undefined') {
                                fn = MY.SelectorFilter[request.fn];
                            }
                        }
                        ...
                        if (typeof fn === 'undefined') {
                            require("functions").log("content script worker called function", request.fn, "but it was not found");
                            return;
                        }
                        var sender = {};
